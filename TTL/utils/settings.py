...
def parse_opts():
    ...
    parser.add_argument('--root_path', type=str, default=r'/home/helinrui/slns/TTL/data', help='path of data folders')
    ...
    parser.add_argument('--resume_path', type=str, default='/home/helinrui/slns/TTL/checkpoint/XBNJ/densenet201_imagenet_-1_1/best.pt',
    ...
    #     # 设置 args.device 为 CUDA 设备
    #     args.device = [3, 4, 5] # 使用索引为 3、4、5 的 CUDA 设备
    #     args.device = torch.device("cuda:0")
    # else:
    #     # 如果没有可用的 CUDA 设备，则设置为 CPU 设备
    ...
    # for chexpert
        ...
        args.root_path = "{}/imgall23diff22/".format(args.root_path)
        # args.weights = torch.Tensor([0.33176124, 0.32179528, 0.34644344]).to(args.device)
        # args.weights = torch.Tensor([0.24593711, 0.23854928, 0.25869250, 0.25682113]).to(args.device)
        # args.weights = torch.Tensor([0.20071700, 0.19468756, 0.39499563, 0.20959981]).to(args.device) imageall23
        args.weights = torch.Tensor([0.23781779, 0.23067386, 0.28316587, 0.24834250]).to(args.device)  # imgall23diff2
        # args.weights = torch.Tensor([0.25018793, 0.24267240, 0.24587953, 0.26126006]).to(args.device)  # imgall23diff2mask
        args.weights = args.weights / torch.min(args.weights)
        args.criterion = torch.nn.CrossEntropyLoss(reduction='mean', weight=args.weights)
        args.classes = 4
