...
def densenet201(pretrained : str ="imagenet", trunc : int=-1, classes : int=4):
    # sainity check
    assert pretrained in SUPPORTED_WEIGHTS, "weights not found"
...
def resnet50(pretrained : str ="imagenet", trunc : int=-1, classes : int=4, args=None, layer_wise=False, freeze=False,attention=False):
    # sainity check
    assert pretrained in SUPPORTED_WEIGHTS, "weights not found"
    ...
    if not layer_wise:
        ...
        net = nn.Sequential(*list(net.children())[:true_trunc+1])  # 按层截断
    ...
    if args != None and args.pooling:
        ...
        linear = nn.Linear(in_features=linear_input_dim,
                    out_features=classes,
                    bias=True)
        torch.nn.init.kaiming_normal_(linear.weight)   # 对新添加的全连接层进行初始化
