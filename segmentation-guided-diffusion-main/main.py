...
def main(
    ...
    if config.segmentation_guided:
        ...
        def transform(examples):
            if img_dir is not None:
                ...
                images = None  # Ensure images is always defined

            images_filenames = examples["image_filename"]

            segs = {
                "seg_{}".format(seg_type): [preprocess_segmentation(image.convert("L")) for image in
    ...
    if ((img_dir is None) and (not segmentation_guided)):
        ...
        eval_dataloader = torch.utils.data.DataLoader(
            torch.utils.data.TensorDataset(torch.zeros(config.eval_batch_size, num_img_channels, config.image_size)),
    ...
    if config.segmentation_guided:
        ...
        if config.segmentation_channel_mode == "single":
            in_channels = num_img_channels + 1  # 单通道模式下，输入通道数为图像通道数加1
        elif config.segmentation_channel_mode == "multi":
            in_channels = num_img_channels + config.num_segmentation_classes  # 多通道模式下，输入通道数为图像通道数加分割类别数
