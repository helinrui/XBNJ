...
def convert_segbatch_to_multiclass(image_shape, segmentations_batch, config, device):
    batch_size, _, height, width = image_shape

    # # 打印键以查看 seg_ 的数量
    # print("Keys in segmentations_batch:", segmentations_batch.keys())

    # 处理 seg_all 掩码
    if 'seg_all' in segmentations_batch:
        segs = segmentations_batch['seg_all'].to(device).expand(batch_size, 1, height, width)
        return segs

    # 如果有其他 seg_ 掩码
    seg_keys = [k for k in segmentations_batch.keys() if k.startswith("seg_") and k != "seg_all"]
    num_segmentation_classes = len(seg_keys)

    segs = torch.zeros((batch_size, num_segmentation_classes, height, width), device=device)

    for k, seg in segmentations_batch.items():
        if k.startswith("seg_") and k != "seg_all":
            seg = seg.to(device)
            seg = seg.expand(batch_size, 1, height, width)
            try:
                seg_idx = int(k.split("_")[1])
            except ValueError:
                print(f"Warning: Skipping invalid key {k}")
                continue
            segs[:, seg_idx] = seg.squeeze(1)
...
def add_segmentations_to_noise(noisy_images, segmentations_batch, config, device):
    ...
    # 处理 seg_all 掩码
    if 'seg_all' in segmentations_batch:
        segs = segmentations_batch['seg_all'].to(device).expand(noisy_images.shape[0], 1, noisy_images.shape[2], noisy_images.shape[3])
...
def evaluate(config, epoch, pipeline, seg_batch=None, class_label_cfg=None, translate=False):
    ...
    if config.segmentation_guided:
        segmentations_batch = {k: v for k, v in seg_batch.items() if k.startswith('seg_')}
        images = pipeline(
            batch_size = config.eval_batch_size,
            seg_batch=seg_batch,
            class_label_cfg=class_label_cfg,
